...
if True:
#    for grid_size in range(10, 20, 2):
    for grid_size in range(20, 100, 2):
...
#        plt.close()
        ...
        mis = []
        mi_stds = []
#        temperature_log_grid_decay = 0.8
#        temperature_samples = 20
        temperature_log_grid_decay = 0.5
        temperature_samples = 10
#        theoretical_transition = 1/2-1/(2*np.sqrt(2))
        theoretical_transition = 0.12
        for i in reversed(range(2*temperature_samples+1)):
            if i == 2*temperature_samples:
                p = theoretical_transition
            if i%2==0:
                p = theoretical_transition - theoretical_transition*temperature_log_grid_decay**(i/2)
            else:
                p = theoretical_transition + (1-theoretical_transition)*temperature_log_grid_decay**((i-1)/2)
            print(i, p)
            if p==0:
                T = -np.inf
                mi, mi_std = np.log(2), 0
            elif p==1:
                T = -np.inf
                mi, mi_std = 0, 0
            else:
        ...
        order = np.argsort(ps)
        ps, Ts, mis, mi_stds = [np.array(x)[order] for x in [ps, Ts, mis, mi_stds]]

        import pickle
        with open("mutual_informations_"+str(grid_size[0]), "wb") as f:
            pickle.dump([grid_size[0], ps, Ts, mis, mi_stds], f)

if False:
    import pickle
    with open("mutual_informations", "rb") as f:
        grid_size, ps, Ts, mis, mi_stds = pickle.load(f)
    grid_size = [grid_size, grid_size]

    print(grid_size, ps, Ts, mis, mi_stds)

    ps_ = [0, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45]
    ...
    ax.plot(ps, mis, 'k-')
    ax.errorbar(ps_, np.log(2)*np.array(p_perc), yerr=np.log(2)*np.array(p_perc_std), fmt='ro', capsize=5, label="Measured upper bound by percolation probability")
    ax.plot(ps_, np.log(2)*np.array(p_perc), 'r-')
    ax.set_xlim([0, 0.5])
